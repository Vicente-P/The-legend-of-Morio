...
public class Mago extends Jugador{
    ...
    private int fuerzaMax = 3;

    private int inteligenciaMax = 10;

    private int energiaMax = 6;

    private int manaMax= 15; 

    public Mago(String nombre,ArrayList<Mision> lista_misiones){ //constructor
        super(nombre,10,0,3,6,10,15,lista_misiones); //herencia
    }
    ...
    public int ataque(){ //metodo abtracto de jugador
        if(getEnergia() == 0){
            System.out.println("No te queda Energia"); //si no te queda energia retorna 0 
            return 0;
        }else{
            int ataque = getFuerza()*(getInteligencia()/4) + getEnergia(); //calculo del daño, se almacena en la variable ataque
            ...
            return ataque; //retorna valor del ataque
    ...
    public int hechizo(){
        if(getMana() == 0){
            System.out.println("No te queda Mana"); //si no te queda mana retorna 0
            return 0;
        }else{
            int hechizo = getMana() + (getFuerza()/4) * getInteligencia();
            setMana(getMana()-5);//setea el mana restandole 5 al actual 
            if(getMana() <= 0){ //cuando detecta que el mana es menor a 0, la setea en 0;
                setMana(0);
            }
            return hechizo; //retorna valor del hehcizo
    ...
    @Override
    public void subir_experiencia(int experiencia){ //metodo abstracto de jugador
        ...
        if(nivelInicial!= nivel()){ //si el nivel actual es diferencte al inicial, se realiza el aumento de los atributos
            vidaMax = vidaMax + 2 * nivel(); //se actualizan las variables extras para almacenar los maximos de cada atributo 
            fuerzaMax = fuerzaMax + (nivel()- nivelInicial); //a este atributo se les suma uno por cada nivel subido 
            energiaMax = energiaMax + (nivel()- nivelInicial);//(nivel() - nivelInicial) esto servira en el caso de subir mas de un nivel
            inteligenciaMax = inteligenciaMax + 3 * nivel();
            manaMax = manaMax + 3 * nivel();
